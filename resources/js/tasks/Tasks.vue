<template>
    <section class="border mb-4" v-if="isModulActiveById">
        <header
        ...
        >
            <div class="flex items-center justify-center">
                ...
                <h3 class="font-semibold cursor-pointer">
                    Úlohy ({{ taskList.length }})
                </h3>
            </div>

            <svg
                v-if="markAsCompleted.length"
                @click="multiUpdateCompleted"
                class="hover:text-gray-300 rounded-md w-5 h-5 fill-current"
                xmlns="http://www.w3.org/2000/svg"
                viewBox="0 0 20 20"
                fill="currentColor"
            >
