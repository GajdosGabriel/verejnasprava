    ...
    <div class="container mx-auto p-6 min-h-screen">
        <div class="md:flex">
            ...
            <div class="w-1/4 md:p-6">
                        ...
                        @can('delete')
                            ...
                            @if($item->vote_disabled)
                                <a href="{{ route('vote.voteEnable', [$item->id, $item->slug]) }}"
                                   class="btn btn-secondary text-xs">Zapnúť hlasovanie</a>
                            @else
                                <a href="{{ route('vote.voteEnable', [$item->id, $item->slug]) }}"
                                   class="btn btn-primary text-xs">Hlasovanie zapnuté</a>
                            @endif
                        @endcan
                ...
                <ul class="mb-10">
                    <li class="font-semibold flex justify-between border-b-2 border-dotted"><span>Hlasovalo:</span>
                        <span>{{ $item->users()->count() }}</span></li>
                    <li class="font-semibold flex justify-between border-b-2 border-dotted"><span>Za:</span>
                        <span>{{ $item->users()->where('vote', 1)->count() }}</span></li>
                    <li class="font-semibold flex justify-between border-b-2 border-dotted"><span>Proti:</span>
                        <span>{{ $item->users()->where('vote', 0)->count() }}</span></li>
                    <li class="font-semibold flex justify-between border-b-2 border-dotted"><span>Zdržal:</span>
