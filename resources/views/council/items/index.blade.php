...
<div class="lg:container lg:flex mx-auto lg:px-6 min-h-screen py-6">
     ...
     <div class="lg:w-3/4 px-4 md:px-6  mb-6">
            <div class="flex justify-between">
                ...
                @can('delete')
                <a href="{{ route('item.create', [ $meeting->id, $meeting->slug ]) }}" class="btn btn-primary text-center text-sm">Nový návrh</a>
                @endcan
            </div>
                                    ...
                                    <nav-horizontal inline-template>
                                       <div class="relative flex items-start">
                                           ...
                                           <div v-if="isOpen" class="absolute w-32 z-10 py-1 flex flex-col border-2 border-gray-300 shadow-md rounded text-sm bg-white">
                                               ...
                                               {{-- Item Up button--}}
                                               <a class="hover:bg-gray-200 px-4 py-1 whitespace-no-wrap" href="{{ route('item.up', [ $item->id, $item->slug]) }}" title="Presunúť položku smerom hore">
                                                   Presúnúť hore
                                               </a>

                                               {{-- Item Down button--}}
                                               <a class="hover:bg-gray-200 px-4 py-1 whitespace-no-wrap" href="{{ route('item.down', [ $item->id, $item->slug]) }}" title="Presunúť položku smerom dole">
                                                   Presúnúť dole
                                               </a>
