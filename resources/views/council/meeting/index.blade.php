...
<x-page.container>
        ...
        <div class=" flex justify-between">
            ...
            <x-page.page-title>
                <x-slot name="title">
                    Zasadnutia: {{ $council->name }}
                </x-slot>

                @can('delete')
                    <a class="btn btn-primary" href="{{ route('meet.create', auth()->user()->id) }}">Nové zasadnutie</a>
                @endcan
            </x-page.page-title>
        </div>
                ...
                @forelse($council->meetings as $council)
                    <tr class="hover:bg-gray-100">
                        <td class="border px-4 py-2">
                            <a href="{{ route('meetings.show', $council->id) }}">
                                {{ $council->start_at->format('d. m. Y') }}
                                <strong>{{ $council->start_at->format('H:i') }}
                                    hod.</strong>
                            </a>
                        </td>
                        <td class="border px-4 py-2">
                                    ...
                                    <div class="relative flex items-start">
                                        ...
                                        <div v-if="isOpen"
                                            ...
                                            <a class="hover:bg-gray-200 px-4 py-1 whitespace-no-wrap"
                                                href="{{ route('meet.edit', [$council->id, $council->slug]) }}"
                                            ...
                                            {{-- Item Down button --}}
                                            <a class="hover:bg-gray-200 px-4 py-1 whitespace-no-wrap"
                                                href="{{ route('meet.delete', [$council->id, $council->slug]) }}"
